...
flowchart TD
    ...
    subgraph Warehouse ["2. Data Warehouse & ML Engine"]
        RAW["gaming_raw.live_session_events"]
        GOLD["gaming_gold.gold_player_360"]
        BQML["BQML Model: gaming_raw.gaming_player_churn_model (ML.PREDICT)"]
    end

    subgraph Governance ["3. Dataplex Knowledge Catalog"]
        Glossary["Business Glossary (Whale Spend, Churn Risk)"]
        Aspects["Aspect Tags: liveops_campaign_policy_aspect & gaming-certified-reward-sku-aspect"]
    end
...
| **Telemetry Pub/Sub Topic** | `retail-data-and-ai-demo/.../games-pubsub.tf` (Step 1) | `google_pubsub_topic.live_telemetry` (`gaming-live-telemetry`). Used for real-time telemetry ingestion. |
...
| **Raw Telemetry Table** | `retail-data-and-ai-demo/.../games-bigquery.tf` (Step 1) | `gaming_raw.live_session_events`. Stores raw incoming telemetry partitioned by day and clustered by `player_id`. |
| **Gold Player 360 Feature Store** | `gamingdatademo/dataform` & `games-bigquery.tf` (Steps 1 & 3) | `gaming_gold.gold_player_360`. Contains player monetization tier (`Whale`, `Dolphin`, `F2P`), LTV, and historical churn metrics. |
| **BQML Churn Prediction Model** | `games-bigquery-procedure.tf` & `deploy-demo.sh` (Step 5) | `gaming_raw.gaming_player_churn_model` trained via `CALL gaming_raw.train_churn_model()`. |
| **BQML ML.PREDICT Stored Procedure** | `games-bigquery-routines/calculate-churn-risk.sql.tftpl` | `gaming_raw.calculate_churn_risk(target_player_id)`. Computes real-time churn score by joining live telemetry with `gold_player_360`. |
| **Dataplex Aspect Tags** | `gamingdatademo/scripts/08_create_churn_guardrail_aspects.py` (Step 4) | Custom aspect types `liveops_campaign_policy_aspect` (Whale max 80% discount) & `gaming-certified-reward-sku-aspect` (`frost_giant_shield_pack`). |
...
2. `verify_aspect_compliance(target_resource, aspect_type)`:
   - Verifies compliance for `liveops_campaign_policy_aspect` and `gaming-certified-reward-sku-aspect` on `gaming_gold.gold_player_360`.
...
- **Implementation Steps**:
  1. Use existing Pub/Sub topic `gaming-live-telemetry` (provisioned by `games-pubsub.tf`).
  2. Ensure zero-code direct BigQuery subscription (`gaming-live-telemetry-bq-sub`) is active and writing to `gaming_raw.live_session_events`.
...
- **Implementation Steps**:
  1. Query `gaming_raw.gaming_player_churn_model` using BigQuery `ML.PREDICT`:
     ```sql
     ...
     ```
  2. Join result with `gaming_gold.gold_player_360` to retrieve player monetization tier (`Whale`, `Dolphin`, `F2P`) and cumulative LTV.
...
- **Implementation Steps**:
  1. Fetch `liveops_campaign_policy_aspect` and `gaming-certified-reward-sku-aspect` from Dataplex Knowledge Catalog REST API (`https://dataplex.googleapis.com/v1/...` or `@google-cloud/dataplex` SDK).
...
- **Implementation Steps**:
  ...
  2. Implement probe checks:
     - **Cloud Pub/Sub**: `pubsubClient.topic('gaming-live-telemetry').exists()`
     - **BigQuery Table**: `bigqueryClient.dataset('gaming_gold').table('gold_player_360').exists()`
     - **BQML Model**: Execute `SELECT * FROM gaming_raw.INFORMATION_SCHEMA.MODELS WHERE model_name='gaming_player_churn_model'`
     - **Dataplex Aspect**: Query Dataplex REST API for `gaming-campaign-policy-aspect`
...
| :--- | :--- | :--- | :--- |
| `// TODO: [Backend Integration] Wire up live gRPC / HTTP2 stream` | `simulatorBridge.ts` | `POST /api/telemetry/stream` | Publishes payload to Pub/Sub topic `gaming-live-telemetry` using `@google-cloud/pubsub`. |
...
| `// TODO: [Backend Probe] Pub/Sub` | `SimulatorDiagnostics.tsx` | `GET /api/diagnostics/gcp` -> `pubsub` | Calls `pubsubClient.topic('gaming-live-telemetry').exists()`. |
| `// TODO: [Backend Probe] BigQuery Table` | `SimulatorDiagnostics.tsx` | `GET /api/diagnostics/gcp` -> `bigquery` | Calls `bigqueryClient.dataset('gaming_gold').table('gold_player_360').exists()`. |
| `// TODO: [Backend Probe] BQML Model` | `SimulatorDiagnostics.tsx` | `GET /api/diagnostics/gcp` -> `bqml` | Executes `SELECT * FROM gaming_raw.INFORMATION_SCHEMA.MODELS WHERE model_name='gaming_player_churn_model'`. |
| `// TODO: [Backend Probe] Dataplex Aspect` | `SimulatorDiagnostics.tsx` | `GET /api/diagnostics/gcp` -> `dataplex` | Queries Dataplex REST API for `gaming-campaign-policy-aspect`. |
...
3. **Verify Pub/Sub to BigQuery Direct Subscription**:
   - Publish a test message to `gaming-live-telemetry` and verify row insertion in `gaming_raw.live_session_events`.
4. **Verify BQML ML.PREDICT Inference**:
   - Execute `CALL gaming_raw.calculate_churn_risk('player_cosmic_whale_42')` in BigQuery console and verify churn score output.
